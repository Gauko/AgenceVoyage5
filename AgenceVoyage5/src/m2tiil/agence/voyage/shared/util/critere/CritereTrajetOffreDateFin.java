...
public class CritereTrajetOffreDateFin extends Critere<Trajet,Date> {
	...
	public boolean correspond(Trajet object) {
		...
		for(Offre o : l2){
			...
			switch(comparator){
			case INFERIOR : {
				if(value1.compareTo(o.getDateFin())<0 ){
					return true;
				}
			}break;
			case SUPERIOR : {
			...
			case BETWEEN : {
				...
				if( value1.compareTo(o.getDateFin())>0 && value2.compareTo(o.getDateFin())<0){
					return true;
				}
			}break;
			case EQUAL : {
				if(value1.equals(o.getDateFin()) ){
					return true;
				}
			}break;
			}
